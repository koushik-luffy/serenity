        ...
        {isLoading && !isVisible ? (
          ...
          >
            ...
            {message.type === 'image' && (
              <div className="relative min-h-32">
                ...
                <img
                  src={message.content}
                  alt="Chat image"
                  className={`h-auto max-h-full max-w-48 rounded object-cover ${!imageLoaded ? 'hidden' : ''}`}
            ...
            {message.type === 'text-with-links' && (
              <div>
                <p className="mb-3 text-sm leading-relaxed" style={{ color: chatStyle.textColor }}>
                  {message.content}
                </p>
...
const MessageWrapper = React.memo<MessageWrapperProps>(({
  ...
  const messageClass = useMemo(() =>
    isLeft ? 'flex items-end gap-3' : 'flex flex-row-reverse items-end gap-3',
    [isLeft]
  );
...
const ChatComponent: React.FC<ChatComponentProps> = ({ config, uiConfig = {} }) => {
  ...
  return (
    <div
      key={key}
      className="relative mx-auto overflow-hidden rounded-[32px] border border-white/60 shadow-[0_24px_80px_rgba(69,75,135,0.14)]"
      style={{
        width: '100%',
        maxWidth: `${ui.containerWidth}px`,
        height: `${ui.containerHeight}px`,
        backgroundColor: ui.backgroundColor
      }}
    >
      <div
        className="pointer-events-none absolute left-0 right-0 top-0 z-10 h-32 rounded-t-[32px]"
        style={{ background: gradientBackground }}
      />

      <div
        ref={containerRef}
        className="scroll-hidden h-full overflow-y-scroll p-8"
        style={{
          scrollbarWidth: 'none',
